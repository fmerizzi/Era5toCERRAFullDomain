...
train_generator = DataGeneratorMemmap("DATA/tas_CERRA_day_1985-2010.nc", "DATA/tas_ERA5_day_1985-2010.nc","t2m","tas", 
                              max_cerra_tas, max_era5_tas, min_cerra_tas, min_era5_tas, inshape = 1072,sequential=False, batch_size=2, unet = True)

test_generator = DataGeneratorMemmap("DATA/tas_CERRA_day_2010-2020.nc", "DATA/tas_ERA5_day_2010-2020.nc","t2m","tas", 
                              max_cerra_tas, max_era5_tas, min_cerra_tas, min_era5_tas, inshape = 1072,sequential=False, batch_size=2, unet = True)
...
with strategy.scope():
    ...
    model = get_full_size_net(image_size, 1, 1, widths, block_depth)
...
class CustomSaveModelCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) % self.save_freq == 0:
            filename = f'tas_unet_{epoch + 1}.weights.h5'
...
save_callback = CustomSaveModelCallback(save_freq = 2, save_path='./weights_d')


save_callback.on_epoch_end(0)
csv_logger = keras.callbacks.CSVLogger('D.log', append=True)

if(training):

    print("training D started")

    model.load_weights("weights_d/tas_unet_448.weights.h5")
    ...
    print("testing D started")
    model.load_weights("weights_d/tas_unet_404.weights.h5")

    sequential_test_generator = DataGeneratorMemmap("DATA/tas_CERRA_day_2010-2020.nc", "DATA/tas_ERA5_day_2010-2020.nc","t2m","tas", 
                              max_cerra_tas, max_era5_tas, min_cerra_tas, min_era5_tas, inshape = 1072,sequential=True, batch_size=16, unet = True)
