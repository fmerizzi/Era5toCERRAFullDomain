...
class CustomSaveModelCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) % self.save_freq == 0:
            filename = f'precipitation_unet_{epoch + 1}.weights.h5'
...
if(training):
    ...
    model.load_weights(save_path+"/precipitation_unet_240.weights.h5")
    
    sequential_test_generator = DataGeneratorMemmap("./DATA/pr_CERRA_day_2010-2020.nc", "./DATA/pr_ERA5_day_2010-2020.nc","tp","pr", 
                              max_cerra_pr, max_era5_pr, min_cerra_pr, min_era5_pr, inshape = 1072,sequential=True, batch_size=2, unet = True)
